...
public class Estudiante {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < estudiantesMain.size(); i++) {
			System.out.println("Nombre: " + estudiantesMain.get(i).getNombre() + ", Edad: "
					+ estudiantesMain.get(i).getEdad() + ", Nota Media: " + estudiantesMain.get(i).getNotaMedia());
		}

		try {
			ArrayList<Estudiante> estudiantes = leerEstudiantes("estudiantes.txt");
			for (int i = 0; i < estudiantes.size(); i++) {
				System.out.println("Nombre: " + estudiantes.get(i).getNombre() + ", Edad: "
						+ estudiantes.get(i).getEdad() + ", Nota Media: " + estudiantes.get(i).getNotaMedia());
			}
		} catch (IOException e) {
