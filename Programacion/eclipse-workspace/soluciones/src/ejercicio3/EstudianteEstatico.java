...
public class EstudianteEstatico {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < estudianteMain.length; i++) {
			System.out.println("Nombre: " + estudianteMain[i].getNombre() + ", Edad: " + estudianteMain[i].getEdad()
					+ ", Nota Media: " + estudianteMain[i].getNotaMedia());
		}
		...
		try {
			...
			for (int i = 0; i < estudiantes.length; i++) {
				System.out.println("Nombre: " + estudiantes[i].getNombre() + ", Edad: " + estudiantes[i].getEdad()
						+ ", Nota Media: " + estudiantes[i].getNotaMedia());
			}
		} catch (IOException e) {
			System.err.println("Error al leer el fichero: " + e.getMessage());
		}
